...
class JSONWriter {
	...
	private String sessionFactoryBeanName = "hibernateSessionFactory";
	 ...
	 */
	public String write(Object object, int maxDeep) throws JSONException {
		try {
			return this.write(object, null, null, maxDeep);
		} finally {
	...
	public String write(Object object, Collection<Pattern> excludeProperties,
			Collection<Pattern> includeProperties, int maxDeep) throws JSONException {
	...
	private void bean(Object object) throws JSONException {
		...
		try {
			...
			for (int i = 0; i < props.length; ++i) {
				...
				//Gabriel: If adicionado para nao ocorrer erros com propriedades
				//que são do hibernate proxy.
				if(accessor == null || name.contains("LazyInitializer")){
					continue;
				}
				...
				} else if (clazz.getName().indexOf("_$$_javassist") > -1) {
					try {
						baseAccessor = Class.forName(
								clazz.getName().substring(0,
	...
	private boolean shouldExcludeProperty(String expr) {
		...
		//verifica se atingiu a profundidade maxima no json de resposta
		String[] splitedExpr = expr.split("\\.");
		int deep = splitedExpr.length-1;
		//log.debug("Profundidade de "+ expr +" igual a "+ deep);
		if(maxDeep >= 0 && deep >=maxDeep) {
			log.debug("Ignorando "+ expr +" devido a profundidade maxima");
			return true;
		}
	...
	private void add(String name, Object value, Method method)
			throws JSONException {
		/**
		if (value != null && value.getClass().getName().contains("Persistent")) {
			// Adiciona valor null para objetos do hibernate nao serem
			// serializados
			this.add('"');
			this.add(name);
			this.add("\":");
